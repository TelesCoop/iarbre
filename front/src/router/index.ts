...
const router = createRouter({
  ...
  routes: [
    ...
    {
      path: "/:dataType(plantability|lcz|vulnerability|plantability_vulnerability|vegestrate)",
      redirect: (to) => {
        return {
    ...
    {
      path: "/:dataType(plantability|lcz|vulnerability|plantability_vulnerability|vegestrate)/:zoom(\\d+)/:lat(-?\\d+\\.\\d{1,4}|\\d+\\.\\d{6,})/:lng(-?\\d+\\.\\d{1,4}|\\d+\\.\\d{6,})",
      redirect: (to) => {
    ...
    {
      path: "/:dataType(plantability|lcz|vulnerability|plantability_vulnerability|vegestrate)/:zoom(\\d+)/:lat(-?\\d+\\.\\d{5})/:lng(-?\\d+\\.\\d{5})",
      name: "mapWithUrlParams",
      component: MapView
    },
    {
